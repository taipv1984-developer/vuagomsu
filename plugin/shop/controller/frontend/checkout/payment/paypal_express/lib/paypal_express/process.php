...
if($_POST)//Post Data received from product list page.
{
	...
	$total_tax_amount 	= 2.58;  //Sum of tax for all items in this order. 
	$handaling_cost 		= 2.00;  //Handling cost for this order.
	$insurance_cost 		= 1.00;  //shipping insurance cost for this order.
	$shipping_discount 	= -3.00; //Shipping discount for this order. Specify this as negative number.
	$shipping_cost 		= 3.00;  //Although you may change the value later, try to pass in a shipping amount that is reasonably accurate.
	...
	//eg : $ItemPrice = $mysqli->query("SELECT item_price FROM product WHERE id = Product_Number");
	$paypal_data ='';
	$item_total_price = 0;
    ...
    foreach($_POST['item_name'] as $key=>$itmname)
   {
	...
	$paypal_product['assets'] = array('tax_total'=>$total_tax_amount, 
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
				...
				$paypal_data.				
				'&NOSHIPPING=0'. //set 1 to hide buyer's shipping address, in-case product that does not require shipping
...
if(isset($_GET["token"])&& isset($_GET["PayerID"]))
{
	...
	$paypal_product = $_SESSION["paypal_product"];
	$paypal_data = '';
	$item_total_price = 0;
	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"])|| "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"]))
	{
		...
		*/
		...
		if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"])|| "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"]))
		{
			...
			VALUES('$buyerName','$buyerEmail','$transactionID','$ItemName',$ItemNumber, $item_total_price,$ItemQTY)");
