...
<form class="form-horizontal form-row-seperated" action="" method="post" enctype="multipart/form-data">
	<div class="portlet light">
		<?php
			...
			if(!$_REQUEST['newsletterEmailTemplateId']){
				$toolbar->addTitle('add', e('Add newsletter email'));
			}
			else{
				$toolbar->addTitle('edit', e('Edit newsletter email'));
			}
...
<script type="text/javascript">
	$(document).ready(function() {
		$("form").submit(function(event){		//required
			var url = "index.php?r=admin/newsletter/email_validate_ajax";
			<?php if(!$_REQUEST['newsletterEmailTemplateId']){ ?>
			var action = 'add';
			<?php } else {?>
			var action = 'edit';
			<?php }?>
			var name = $('input[name="newsletterEmailModel.key"]').val();
			var newsletterEmailTemplateId = '<?php echo $newsletterEmailInfo->newsletterEmailTemplateId?>';
			
			$.ajax({
				type : 'post',
				url : url,
				data:{'action': action, 'name': name, 'newsletterEmailTemplateId': newsletterEmailTemplateId},
				async : false,
				complete : function(){
			    },
