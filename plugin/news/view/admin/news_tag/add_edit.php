<?php
	$newsTagInfo = $_REQUEST['newsTagInfo'];
?>
	...
	<div class="portlet light">
		<?php
			...
			if(!$_REQUEST['newsTagId']){
				$toolbar->addTitle('add', e('Add news tag'));
			}
			else{
				$toolbar->addTitle('edit', e('Edit news tag'));
			}
...
<script type="text/javascript">
	$(document).ready(function() {
		$("form").submit(function(event){		//required
			var url = "index.php?r=admin/news_tag/validate_ajax";
			<?php if(!$_REQUEST['newsTagId']){ ?>
			var action = 'add';
			<?php } else {?>
			var action = 'edit';
			<?php }?>
			var name = $('input[name="newsTagModel.name"]').val();
			var newsTagId = '<?=$newsTagInfo->newsTagId?>';
			
			$.ajax({
				type : 'post',
				url : url,
				data:{'action': action, 'name': name, 'newsTagId': newsTagId},
				async : false,
				complete : function(){
			    },
