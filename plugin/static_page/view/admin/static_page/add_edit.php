...
<form class="form-horizontal form-row-seperated" action="" method="post" enctype="multipart/form-data">
	<div class="portlet light">
		<?php
			...
			if(!$_REQUEST['staticPageId']){
				$toolbar->addTitle('add', e('Add static page'));
			}
			else{
				$toolbar->addTitle('edit', e('Edit static page'));
			}
		...
		<div class="panel panel-default">
			...
			<?php
				...
				$settingForm = array(
					...
					'image'	=> array('type' => 'file', 'value' => $staticPageInfo->image, 'action' => true),
					'summary'	=> array('type' => 'textarea', 'class' => 'ckeditor_mini'),
					'content'	=> array('type' => 'textarea', 'class' => 'ckeditor'),
					'status'	=> array('type' => 'select', 
							'value' => $staticPageInfo->status, 'options' => ArrayHelper::getAD()),
				);
				$settingValue = $staticPageInfo;
				$settingAll = array(
...
<script type="text/javascript">
	$(document).ready(function() {
		$("form").submit(function(event){		//required
			var url = "index.php?r=admin/static_page/validate_ajax";
			<?php if(!$_REQUEST['staticPageId']){ ?>
			var action = 'add';
			<?php } else {?>
			var action = 'edit';
			<?php }?>
			var title = $('input[name="staticPageModel.title"]').val();
			var staticPageId = '<?=$staticPageInfo->staticPageId?>';
			
			$.ajax({
				type : 'post',
				url : url,
				data:{'action': action, 'title': title, 'staticPageId': staticPageId},
				async : false,
				complete : function(){
			    },
